...
Here's a new version of "getGithubReports" that invokes this rewritten function.

[source,scala]
-------------------------------------------------------------------------------
scala> def getGithubReports(urbs: List[(String,String,String)]) = {
...
c) We'll need yet another version of "getGithubReports", this time with futures.
...
Let's update the "report" function and also print out the user, repo and branch information.
...
scala> def getGithubReport(urb: (String,String,String)): String = {
     ...
     |   val entries = xmlToEntryList(xml).toList
     |   val branchInfo = s"branch:  ${urb._2}:${urb._3}\n"
     |   val formattedEntries = entries flatMap report map (branchInfo + _)
     |   val title = s"Github commit activity for ${urb._2}:${urb._3}"
     |   title :: formattedEntries mkString ("\n" + "=" * 80 + "\n")
     | }
getGithubReport: (urb: (String, String, String))String
